 ...
 *   \date 2023-2025
 *   Allow passing handle of MUMPS sparse linear solver
 *   through SQProblemSchur constructor. 
 *   Enable usage of SPRAL linear solver.
 * 
 */
...
returnValue SQProblemSchur::repairSingularWorkingSet( )
{
	...
	for (int_t i_ = 0; i_ < nVar_; i_++){
		if (lb[i_] == ub[i_]) return RET_KKT_MATRIX_SINGULAR;
	}
