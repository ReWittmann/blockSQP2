...
\end{itemize}
thoug gurobi and qpalm are experimental. \qpOASES\ is required if sparse nonconvex QPs are to be allowed for better convergence. qpOASES itself requires a sparse linear solver such as \texttt{MA57} from \texttt{HSL} or \texttt{MUMPS}.
\subsection{Building the python and julia interfaces}
...
Most QP solvers as well as the used interface libraries provide CMake project files. A CMakeLists.txt build specification is provided for blockSQP that takes care of most of the above build requirements. In addition, independent unfinished CMakeLists.txt are provided for each of the C++ package, the python interface and the julia interface.\\
The default build downloads \texttt{MUMPS} \cite{amestoy2000mumps} to build \qpOASES\ (\url{https://projects.coin-or.org/qpOASES}) with it. It is the users responsibility to ensure they are eligible to download and use \texttt{MUMPS}. The user needs to specify wether the python and julia interface should be built. In addition, they can provide the path to the python interpreter that should be built for. A path to the CxxWrap library CMake files needs to be provided to build the julia interface.\\
...
\subsubsection{Class \texttt{Condenser}}
TODO
...
\item jac\_g\_row [np.ndarray[np.int32](jac\_g\_nnz,)] - row indices of constraint Jacobian in CCS format
\item jac\_g\_colind [np.ndarray[np.int32]](nVar + 1,) - column start indices of constraint Jacobian in CCS format. 
\end{itemize}
...
\end{itemize}
and set\_blockIndex with hessBlock\_index [np.ndarray[np.int32]]. A feasibility restoration heuristic can be added as the attribute\\ continuity\_restoration [np.ndarray[np.float64]](nVar,) $\rightarrow$ np.ndarray[np.float64](nVar, )].\\
\textbf{Important.} prob.complete() must be called once all data has been provided to the problem to finalize it. \\~\\
...
\texttt{convStrategy}			& Choice of convexification strategy							& 0			\\
								...
								& 2: Add multiple of identity to free indices of first Hessian					&\\\hline
\end{longtable}
