...
\title{[WIP] Updated \blockSQP\ user's manual}
...
\subsection{Building the C++ package}
 \blockSQP\ is built from the source files in the src directory. The following dependencies need to be linked.
\begin{itemize}
\item a BLAS library.
\item a QP solver.
\end{itemize}
\textbf{Linking a QP solver}\\
In addition to linking the shared/ static binary of a QP solver, the preprocessor flag QPSOLVER\_[QP solver name (capital letters)] needs to be set. Currently available are
\begin{itemize}
	\item \qpOASES\ [convex and nonconvex QPs, dense and sparse matrices]
	\item gurobi [convex QPs, dense and sparse matrices]
	\item qpalm [convex QPs, poor performance for nonconvex QPs, dense and sparse matrices]
\end{itemize}
thoug gurobi and qpalm are experimental. \qpOASES\ is required if sparse nonconvex QPs are to be solved. In this case qpOASES itself requires a sparse linear solver such as \texttt{MA57} from \texttt{HSL} or \texttt{MUMPS}. See the respective manuals of the QP solvers.
~\\~\\
\textbf{Building with qpOASES and MUMPS for parallel QP solution}\\
The MUMPS sparse linear solver is \textbf{NOT THREADSAFE}. 

\subsection{Interfaces to other languages}
\textbf{Python.} The python interface code is located in a separate folder of the same name. It requires the pybind11 library, whose headers need to be in the include path and which itself requires the python headers. The pybind11 documentation recommends using CMake to build the python module. Setting PYTHON\_EXECUTABLE to a path to a python interpreter before using find\_package(pybind11) allows specifying the python environment to build for. pybind11\_DIR can be set to the path to the pybind11 cmake files to ensure the right pybind11 installation is found.\\~\\
\textbf{Julia.} The julia interface code similarly depends on the libcxxwrap-julia  library on the C++ side and the corresponding CxxWrap.jl library on the julia side. Using CMake is again recommended for building.\\~\\
As an alternative, the julia interface can be built on blockSQP\_jll from binaryBuilder.jl via the blockSQP.jl [TODO MAKE AVAILABLE] package.
\subsection{Building via CMake}
Most QP solvers, as well as the used interface libraries provide CMake project files. A \mbox{CMakeLists.txt} build specification is provided for blockSQP that takes care of most of the above build requirements. In addition, separate \mbox{CMakeLists.txt} templates are provided for each of the C++ package, the python interface and the julia interface.\\
The default build downloads \texttt{MUMPS} \cite{amestoy2000mumps} to build \qpOASES\ (\url{https://projects.coin-or.org/qpOASES}) with it. It is the users responsibility to ensure they are eligible to download and use \texttt{MUMPS}. The user needs to specify whether the python and julia interface should be built via the arguments -DPYTHON\_INTERFACE=ON / -DJULIA\_INTERFACE=ON. In addition, they can provide the path to the python interpreter that should be built for (-DPYTHON\_INTERPRETER=...). A path to the CxxWrap library CMake files needs to be provided to build the julia interface (-DCXXWRAP\_PATH=...).\\
See the \texttt{readme.md} for further details.
%
%
%\begin{enumerate}
...
The condenser class of blockSQP requires the following structs.
\begin{itemize}
...
%---------------------------------------------------------------------72
\subsubsection{Class \method\ and \Cmethod}
%---------------------------------------------------------------------72
...
\end{itemize}
In addition, the \texttt{SCQPmethod} (Sequential Condensed Quadratic Programming) subclass takes an instance of \texttt{Condenser} for the given problem as fourth argument.\\
~\\
...
...
prob = Problemspec() \# blockSQP\_pyProblem()
\end{lstlisting}
...
\end{itemize}
If sparse derivatives should be used, the option sparseQP must be set $> 0$ and the make\_sparse method has to be called with the following arguments
\begin{itemize}
...
The julia interface requires the compiled jul\_blockSQP.so binary and the julia side module blockSQP.jl. It similar to the Python interface and accessed in the following way
\begin{lstlisting}
using blockSQP
...
\end{lstlisting}
\textbf{Important:} Mind the sign of the returned multipliers, see Section \ref{LAG_DEF_SECT}.\\~\\
\text{}[WIP] In the future, \blockSQP\ may also be called through \texttt{Optimization.jl}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%72
\section{Options and parameters}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%72
In this section we describe the most important options that are passed to \blockSQP\ through the \texttt{SQPoptions} class. For the remaining options look into the options.hpp header file. 
...
%
\texttt{sparse\_mode}				& Use dense/sparse callbacks 						& False			\\\hline
%								
\texttt{exact\_hess\_usage}			& Request exact Hessian in callbacks						& 0			\\
								& 0: Disabled											& \\
								& 1: Request last block										& \\
								& 2: Request all blocks 											&\\\hline
%
\texttt{hess\_approximation}		& 						& 1			\\
								& 0: Scaled identity											& 			\\
								& 1: SR1													& 			\\
								& 2: damped BFGS											& 			\\\hline
%
\texttt{fallback\_approximation}		& See hess\_approximation, has to be positive definite							& 2			\\\hline
%
...
\texttt{max\_QP\_it}				& Maximum number of QP iterations per							& 5000		\\
								& SQP iteration							&			\\\hline
%
\texttt{max\_QP\_seconds}				& Maximum time in seconds for \qpOASES\ per						& 10000.0		\\
								& SQP iteration							&			\\\hline
\end{longtable}
...
%
\texttt{sizing\_strategy}			& Hessian sizing strategy											& 2			\\
								& 0: off															&			\\
								& 1: Shanno-Phua													&			\\
...
%
\texttt{fallback\_sizing\_strategy}	& fallback Hessian sizing strategy							& 4			\\\hline
%
&&\\[-0.2cm]
\textbf{Advanced options}					&						&\\\hline
