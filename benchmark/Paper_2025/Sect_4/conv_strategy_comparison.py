...
opt_CS_conv = blockSQP2.SQPoptions(
    max_conv_QPs = 4,
    conv_strategy = 'convex_combinations',
    max_filter_overrides = 0
)

opt_CS_full = blockSQP2.SQPoptions(
    max_conv_QPs = 4,
    conv_strategy = 'full_regularization',
    max_filter_overrides = 0,
)

opt_CS_red = blockSQP2.SQPoptions(
    max_conv_QPs = 4,
    conv_strategy = 'reduced_regularization',
    max_filter_overrides = 0,
)

opt_CS_red_par = blockSQP2.SQPoptions(
    max_conv_QPs = 4,
    conv_strategy = 'reduced_regularization',
    par_QPs = True,
...
Experiments = [
                ...
                (opt_CS_conv, "convex combinations (sequential)"),
                (opt_CS_full, "full regularization (sequential)"),
                (opt_CS_red, "reduced regularization (sequential)"),
                (opt_CS_red_par, "reduced regularization (parallel)"),
               ]

plot_folder = cD / Path("out_conv_strategy_comparison_final")
